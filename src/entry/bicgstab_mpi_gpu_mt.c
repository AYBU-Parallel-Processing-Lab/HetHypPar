...
// prof_sync callback for GPU ranks (default stream); CPU ranks pass NULL.
...
//-------------------------------------------------------------------------------------------------------    
    ...
    // Unified profiling (env HHP_PROF), alongside the untouched PROFILE_* lines.
    // Init per-branch with the right sync callback. PF_DOT includes MPI_Allreduce.
    Prof g_P;
    ...
    if (isgpu){ // GPU PROCESS
        ...
        // Row-parallel local SpMV needs a CSR copy of the local block (one-time).
        CSR loc_csr = SHARD_loc_CSC_to_CSR(A.loc);
    ...
    //-------------------------------------------------------------------------------------------------------
        ...
        for (size_t i = 0; i < niters; i++)
        {
            ...
            double t_spmv0 = omp_get_wtime();
            MPI_SHARD_CSC_mpi_spmxv_omp(A, loc_csr, P, V, MPI_COMM_WORLD, &sp);
            double t_spmv1 = omp_get_wtime();
            prof_lap(&g_P, PF_SPMV);
            ...
            double t_spmv2 = omp_get_wtime();
            MPI_SHARD_CSC_mpi_spmxv_omp(A, loc_csr, S, T, MPI_COMM_WORLD, &sp);
            double t_spmv3 = omp_get_wtime();
            prof_lap(&g_P, PF_SPMV);
    ...
    // Unified hhp_prof report (per-rank label so per-iteration TSVs don't collide).
    {
        ...
        snprintf(solver_label, sizeof(solver_label), "mpi-gpu-mt-r%d", mpi_rank);
