...
__global__ void kernel_update_P(int n, double *P, const double *R, const double *V, double beta, double omega) {
...
__global__ void kernel_update_S(int n, double *S, const double *R, const double *V, double alpha) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        S[i] = R[i] - alpha * V[i];
    }
...
__global__ void kernel_update_X_R(int n, double *X, double *R, const double *P, const double *S, const double *T, double alpha, double omega) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
//-------------------------------------------------------------------------------------------------------    
    ...
    Iter_Profile *iprof = (Iter_Profile *)calloc(niters, sizeof(Iter_Profile));
    ...
    if (isgpu){ // GPU PROCESS
        ...
        for (size_t i = 0; i < niters; i++)
        {
            ...
            rho = temp_rho;
            int blockSize = 256;
            int numBlocks = (P.nvals + blockSize - 1) / blockSize;
            kernel_update_P<<<numBlocks, blockSize>>>(P.nvals, P.vals, R.vals, V.vals, beta, omega);
            ...
            alpha = rho/temp_rv;
            int blockSizeS = 256;
            int numBlocksS = (S.nvals + blockSizeS - 1) / blockSizeS;
            kernel_update_S<<<numBlocksS, blockSizeS>>>(S.nvals, S.vals, R.vals, V.vals, alpha);
            ...
            int blockSizeX = 256;
            int numBlocksX = (dX.nvals + blockSizeX - 1) / blockSizeX;
            kernel_update_X_R<<<numBlocksX, blockSizeX>>>(dX.nvals, dX.vals, R.vals, P.vals, S.vals, T.vals, alpha, omega);
            double tol;
    ...
    //-------------------------------------------------------------------------------------------------------
        ...
        for (size_t i = 0; i < niters; i++)
        {
            ...
            for (unsigned int j = 0; j < P.nvals; j++) {
                P.vals[j] = R.vals[j] + beta * (P.vals[j] - omega * V.vals[j]);
            }
            ...
            #pragma omp parallel for
            for (unsigned int j = 0; j < S.nvals; j++) {
                S.vals[j] = R.vals[j] - alpha * V.vals[j];
            }
            ...
            #pragma omp parallel for
            for (unsigned int j = 0; j < X.nvals; j++) {
                X.vals[j] = X.vals[j] + alpha * P.vals[j] + omega * S.vals[j];
                R.vals[j] = S.vals[j] - omega * T.vals[j];
            }
