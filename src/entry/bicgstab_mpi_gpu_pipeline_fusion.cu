...
__global__ void kernel_pipe_update_part1(
    int n,
    double *P, double *S, double *Z, double *Q, double *Y,
    const double *P_prev, const double *S_prev, const double *Z_prev, const double *V_prev,
    const double *R, const double *W, const double *T,
    double alpha, double beta, double omega)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        P[i] = R[i] + beta * (P_prev[i] - omega * S_prev[i]);
        S[i] = W[i] + beta * (S_prev[i] - omega * Z_prev[i]);
        Z[i] = T[i] + beta * (Z_prev[i] - omega * V_prev[i]);
        Q[i] = R[i] - alpha * S[i];
        Y[i] = W[i] - alpha * Z[i];
    }
}

__global__ void kernel_pipe_update_part2(
    int n,
    double *X, double *R_next, double *W_next,
    const double *P, const double *Q, const double *Y, const double *T, const double *V,
    double alpha, double omega)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        X[i] = X[i] + alpha * P[i] + omega * Q[i];
        R_next[i] = Q[i] - omega * Y[i];
        W_next[i] = Y[i] - omega * (T[i] - alpha * V[i]);
    }
}
...
//-------------------------------------------------------------------------------------------------------    
    ...
    if (isgpu){ // GPU PROCESS
        ...
        for (size_t i = 0; i < niters; i++)
        {
            ...
            int blockSize2 = 256;
            int numBlocks2 = (dX.nvals + blockSize2 - 1) / blockSize2;
            kernel_pipe_update_part2<<<numBlocks2, blockSize2>>>(dX.nvals, dX.vals, R_next.vals, W_next.vals, P.vals, Q.vals, Y.vals, T.vals, V.vals, alpha, omega);
