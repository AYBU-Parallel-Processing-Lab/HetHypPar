...
// SpMV-only CPU benchmark.
// Reads matrix A and input vector X, repeatedly computes Y = A*X, and times
// the multiplication. If X was prepared via prepare_spmv_input.m, Y should
// equal [1, 2, ..., n], which makes correctness trivial to eyeball/verify.
...
static char doc[] = "SpMV-only CPU benchmark -- repeatedly computes Y = A*X and times it";
...
int main(int argc, char *argv[]) {
    ...
    // Warm-up SpMV (first call pays page-fault / cache-fill costs)
    CSR_spmxv_seq(A, X, Y);
