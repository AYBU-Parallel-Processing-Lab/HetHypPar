...
// The CSR matrix produced by this algorithm is UNSORTED
...
void readMatrixMarketFromFile(buMatrix *A, char *fname) {
  ...
  // A->ia[M] is 0
  // so A->ia[M] is equal to A->ia[M-1]
  for (i = 1; i <= M; i++)
    A->ia[i] += A->ia[i - 1];
...
static inline void readMatrixPartsFromMemory(buMatrix *A, int partScheme,
                     int const*const inpart, int const*const outpart) {
  int numProcs;
...
void readMatrixMarketFromMemory(buMatrix *Aout, const CSR Ain, int partScheme, int const*const inpart,
  ...
  if (myId == 0) {
    ...
    readMatrixFromCSR( &Ain, &bigA);

    readMatrixPartsFromMemory(&bigA, partScheme, inpart, outpart);
