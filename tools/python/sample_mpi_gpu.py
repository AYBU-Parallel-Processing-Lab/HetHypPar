...
rankfile = Path("data/rankfile/16cpu_1gpu.rankfile")
is_gpu_file = Path("data/is_gpu/g17_17.txt")
...
def check_existing_results(log_dir: Path, part_files_hash: str) -> bool:
    """Check if results already exist for this set of partition files."""
    if not log_dir.exists():
        return False

    # Look for TSV files matching the hash pattern
    pattern = f"benchmark_results_*_{part_files_hash}.tsv"
...
def run_mpi_benchmark(
    matrix_name: str, part_file: Path, iterations: int, pbar_part: tqdm
) -> dict:
    """Run the MPI benchmark for a single partition file."""
    mat_basedir = Path("data/matrices") / matrix_name
    part_file_path = mat_basedir / "in" / "part" / part_file.name
...
def run_mpi_gpu_benchmark(
        matrix_name: str, part_file: Path, is_gpu: Path, iterations: int, pbar_part: tqdm
) -> dict:
    """Run the MPI benchmark for a single partition file."""
    mat_basedir = Path("data/matrices") / matrix_name
    part_file_path = mat_basedir / "in" / "part" / "hybrid-cpu-gpu" / part_file.name
...
def process_matrix(
    matrix_name: str, pbar_matrix: tqdm, pbar_part: tqdm
) -> pd.DataFrame:
    """Process CPU, GPU, and all partition files for a single matrix."""
    mat_basedir = MATRICES_DIR / matrix_name
    part_dir = mat_basedir / "in" / "part"
    part_gpu_dir = mat_basedir / "in" / "part" / "hybrid-cpu-gpu-2"
    log_dir = mat_basedir / "log" / "hybrid-2"
    ...
    if check_existing_results(log_dir, part_gpu_files_hash):
        pbar_matrix.write(
            f"⏭ Skipping {matrix_name}: Results already exist (hash: {part_files_hash})"
        )
        return pd.DataFrame()

    if check_existing_results(log_dir, part_files_hash):
        pbar_matrix.write(
            f"⏭ Skipping {matrix_name}: Results already exist (hash: {part_files_hash})"
        )
        return pd.DataFrame()
    ...
    for part_file in part_files:
        try:
            metrics = run_mpi_benchmark(matrix_name, part_file, ITERATIONS, pbar_part)
            metrics["matrix_name"] = matrix_name
            results.append(metrics)
            pbar_part.update(1)
        except Exception as e:
    ...
    for part_gpu_file in part_gpu_files:
        try:
            metrics = run_mpi_gpu_benchmark(matrix_name, part_gpu_file, is_gpu_file, ITERATIONS, pbar_part)
            metrics["matrix_name"] = matrix_name
            results.append(metrics)
            pbar_part.update(1)
        except Exception as e:
    ...
    tsv_file = log_dir / f"benchmark_results_{timestamp}_{part_files_hash}_{part_gpu_files_hash}.tsv"
...
def main():
    ...
    try:
        for matrix_name in matrix_names:
            ...
            df = process_matrix(matrix_name, pbar_matrix, pbar_part)
            if not df.empty:
