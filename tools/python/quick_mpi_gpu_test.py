#!/usr/bin/env python3
"""Quick MPI+GPU test: partition a matrix with patpart, then run bicgstab-mpi-gpu.

Can be used as a CLI tool or imported as a library:
    from quick_mpi_gpu_test import run
    result = run(weight_file=Path("data/weights/hybrid-cpu-gpu/w100-100_17.txt"))
"""
...
BICGSTAB_MPI_GPU_BINARY = PROJECT_ROOT / "build" / "bicgstab-mpi-gpu"
...
def get_is_gpu_file(nranks: int, gpu_rank: int) -> Path:
    """Get or create an is_gpu file with GPU on the specified rank."""
    IS_GPU_DIR.mkdir(parents=True, exist_ok=True)
    path = IS_GPU_DIR / f"{nranks}r_gpu{gpu_rank}.txt"
    if path.exists():
        print(f"is_gpu file already exists, reusing: {path}")
        return path
    lines = ["1" if r == gpu_rank else "0" for r in range(nranks)]
    path.write_text("\n".join(lines) + "\n")
    print(f"is_gpu file created: {path}")
    return path
...
def run(
    ...
    """Run MPI+GPU solver with the given weight file.

    Args:
        gpu_rank: 0-indexed rank for GPU. Defaults to last rank.
        rankfile: Optional OpenMPI rankfile for CPU pinning. When provided,
                  replaces the default '-bind-to core' with '--rankfile <path>'.

    Returns dict with keys: stdout, stderr, returncode, metrics, part_path, nranks, gpu_rank.
    ...
    gpu_rank = gpu_rank if gpu_rank is not None else nranks - 1
    if gpu_rank < 0 or gpu_rank >= nranks:
        raise ValueError(f"gpu_rank {gpu_rank} out of range [0, {nranks - 1}]")

    if not BICGSTAB_MPI_GPU_BINARY.exists():
        raise FileNotFoundError(f"Solver binary not found: {BICGSTAB_MPI_GPU_BINARY}")
    ...
    is_gpu_path = get_is_gpu_file(nranks, gpu_rank)
    print(f"GPU on rank {gpu_rank}")
    ...
    cmd = [
        *mpi_args,
        ...
        "-n", str(iterations),
        "-p", str(part_path),
        "-o", str(mat_basedir / "out" / "X_mpi_gpu.txt"),
...
def main():
    parser = argparse.ArgumentParser(description="Quick MPI+GPU test runner")
    ...
    parser.add_argument("-g", "--gpu-rank", type=int, default=None)
