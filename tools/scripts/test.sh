...
run_cmd ./build/bicgstab-cpu \
  -m /matrices/${MATNAME}.mtx \
  -n ${ITERATIONS} \
  -o ${MAT_BASEDIR}/out/X_seq.txt \
  -x ${MAT_BASEDIR}/in/X_init.txt \
  -y ${MAT_BASEDIR}/in/B.txt

echo
echo "Running GPU version:"
run_cmd ./build/bicgstab-gpu \
  -m /matrices/${MATNAME}.mtx \
  -n ${ITERATIONS} \
  -o ${MAT_BASEDIR}/out/X_gpu.txt \
  -x ${MAT_BASEDIR}/in/X_init.txt \
  -y ${MAT_BASEDIR}/in/B.txt
...
run_cmd mpirun --report-bindings -bind-to core -n ${MPI_PROCS} ./build/bicgstab-mpi-gpu \
  -m /matrices/${MATNAME}.mtx \
  -n ${ITERATIONS} \
  -p ${PART_FILE_PATH} \
  -o ${MAT_BASEDIR}/out/X_mpi_gpu.txt \
  -x ${MAT_BASEDIR}/in/X_init.txt \
  -y ${MAT_BASEDIR}/in/B.txt \
  -g ${IS_GPU_FILE}
